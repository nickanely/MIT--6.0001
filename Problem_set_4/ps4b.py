...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        if shift is 4 , char should be shifted by 4 elements
        '''
        shifted = shift % 26
        dict = {}
        for char in string.ascii_lowercase + string.ascii_uppercase:
            if 97 <= (ord(char) + shifted) <= 122 or 65 <= (ord(char) + shifted) <= 90:
                dict[char] = chr(ord(char) + shifted)
            else:
                dict[char] = chr(ord(char) + shifted - 26)
    ...
    def apply_shift(self, shift):
        ...
        '''
        shifted = shift % 26
        dict = self.build_shift_dict(shifted)
        result = ""
        for char in self.message_text:
            if char.isalpha():
                result += dict.get(char, "Index out of bound")
            else:
                result += char
        return result
